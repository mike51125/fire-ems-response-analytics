...
# FILE UPLOADS
# =====================================================
call_log_upload = st.file_uploader("Upload April Call Log CSV", type=["csv"])
monthly_upload = st.file_uploader("Upload Monthly Incident Numbers CSV", type=["csv"])
overlap_upload = st.file_uploader("Upload Overlapping Incidents Excel File", type=["xlsx"])

if call_log_upload is None or monthly_upload is None or overlap_upload is None:
    st.info("Upload all three files to run the dashboard.")
    st.stop()

calllog = pd.read_csv(call_log_upload)
monthly = pd.read_csv(monthly_upload)
